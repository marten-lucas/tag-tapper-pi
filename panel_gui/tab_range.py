...
import logging
try:
    from panel_gui.wifi_monitor import WiFiMonitor
    WIFI_MONITOR_AVAILABLE = True
    logging.info("WiFiMonitor import successful")
except Exception as e:
    WIFI_MONITOR_AVAILABLE = False
    WiFiMonitor = None
    logging.error(f"WiFiMonitor import failed: {e}", exc_info=True)
...
class TabRange:
    ...
    def draw(self, surface, rect, app, styles, fonts):
        ...
        # Get WiFi state for roaming/bandsteering info (optional) - cached to reduce CPU load
        wifi_state = None
        now = time.time()
        if self.wifi_monitor:
            ...
            wifi_state = self.wifi_state_cache
